...
def generator_zip(seq1, seq2, *more_seqs):
    ...
    if more_seqs==():
        while True:
            try:
                yield (seq1.__next__(),seq2.__next__())
            except StopIteration:
                return
    else:
        more_seq_iter = []
        for seq in more_seqs:
            more_seq_iter.append(seq.__iter__())
        while True:
            try:
               z=(seq1.__next__(),seq2.__next__())+tuple([more_seq_iter[i].__next__() for i in range(len(more_seq_iter))])
               yield z
            except StopIteration:
                return
